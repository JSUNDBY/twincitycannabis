...
async function handleAdminDispensaries(request, env, cors) {
  ...
  try {
    ...
    // Menu platform + visibility per shop (index:platforms, synced by
    // scripts/sync_platforms_to_kv.js): which ecommerce platform their site
    // runs (DUTCHIE, DISPENSARY.SHOP, ...) and whether their menu is LIVE on
    // TCC or DARK (listed but no price data = missing the marketing).
    const platforms = (await env.TCC_OVERRIDES.get('index:platforms', { type: 'json' })) || {};
      ...
      '<td class="pipe-disp">' +
        ...
        ((d.platform || d.menu) ?
          '<div class="sub" style="margin-top:4px">' +
            (d.platform ? '<span style="border:1px solid #444;border-radius:4px;padding:0 5px;font-size:0.68rem;letter-spacing:0.5px">' + esc(d.platform) + '</span> ' : '') +
            (d.menu === 'LIVE'
              ? '<span style="color:#22c55e;font-size:0.72rem">menu live · ' + (d.menuCount || 0) + ' products</span>'
              : d.menu === 'DARK'
