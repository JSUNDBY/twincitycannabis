#!/usr/bin/env python3
"""
Merge Blaze/Tymber (MN THC Co, Winona Dispensary) product data into data.js.

Same pattern as merge_jane_data.py: removes Weedmaps prices for
Blaze-scraped dispensaries, then adds verified prices from the
dispensary's own website.

Run after blaze_scrape.py:
    python3 scraper/blaze_scrape.py
...
BLAZE_FILE = DATA_DIR / "blaze_products.json"
DATA_JS = Path(__file__).parent.parent / "js" / "data.js"


def main():
    if not BLAZE_FILE.exists():
        print("No blaze_products.json found, skipping")
        return
    ...
    if not products:
        print("blaze_products.json is empty, skipping")
        return

    blaze_dispensaries = set(p["dispensary_id"] for p in products)
    ...
    for p in products:
        ...
        # Re-normalize the category — Blaze source data can mislabel flower
        ...
        if normalized_cat == "EXCLUDE":
            # Unlike dispensary.shop/Meadow, Blaze categories are the store's
    ...
    print(f"Loaded {len(products)} Blaze products -> {len(grouped)} unique" + (f" [excluded {excluded_count} mislabeled/junk]" if excluded_count else ""))
    print(f"Blaze dispensaries: {', '.join(sorted(blaze_dispensaries))}")
    ...
    # This preserves images for products that Blaze scrapes without images.
    wm_images = {}
    ...
    # Remove old Blaze entries (id starts with 'bz'). Uses brace-counting
    # because the simple `[^{}]*` regex fails on entries with nested braces.
    from merge_jane_data import _strip_entries_with_id_prefix
    products_text, old_blaze = _strip_entries_with_id_prefix(products_text, 'bz')
    if old_blaze:
        print(f"Removed {old_blaze} old Blaze entries")

    # Remove Blaze dispensary prices from Weedmaps entries
    removed_prices = 0
    for disp_id in blaze_dispensaries:
    ...
    print(f"Removed {removed_prices} Weedmaps price entries for Blaze dispensaries")
    ...
    for key, p in grouped.items():
        ...
        # Use Weedmaps image as fallback if Blaze didn't provide one
        image = p.get("image") or wm_images.get(p["name"].lower().strip(), "")
        image_escaped = image.replace("'", "\\'")

        entry = (
            f"{{ id: 'bz{len(blaze_entries):04d}', "
            f"name: '{name_escaped}', "
    ...
    print(f"Added {len(blaze_entries)} Blaze products to data.js")
