...
// Price check-ins: shoppers report register prices ("what did you pay?").
// Approved reports render client-side from the worker (5-min edge cache), so
...
const checkinBlock = (d, products) => {
  ...
  return `
<div id="checkins-section" data-slug="${esc(d.id)}">
  <h2 id="checkins-title" hidden>What shoppers actually paid here</h2>
  <div id="checkins-list"></div>
  <p style="margin:1rem 0 .4rem"><button type="button" class="cta" id="checkin-open" style="border:none;cursor:pointer;font:inherit">💵 What did you pay? Help other shoppers</button></p>
  <p class="text-xs" style="color:var(--text-muted,#8b909a);font-size:.78rem;margin:0 0 1rem">Menu prices are one thing — the register is the truth. Your report helps everyone shop smarter. Reports are reviewed before they appear.</p>
  <form id="checkin-form" hidden style="max-width:440px;background:var(--bg-card,rgba(255,255,255,0.03));border:1px solid var(--border,#22302a);border-radius:12px;padding:1rem;margin-bottom:1.2rem">
    <input list="checkin-products" id="checkin-product" placeholder="What did you buy? *" maxlength="120" required style="width:100%;margin-bottom:.6rem;padding:.6rem .75rem;background:var(--bg-secondary,#0b120e);color:var(--text-primary,#f5f6f8);border:1px solid var(--border,#22302a);border-radius:8px;font:inherit">
    <datalist id="checkin-products">${datalist}</datalist>
    <input type="number" id="checkin-price" placeholder="What you paid, before tax ($) *" min="1" max="500" step="0.01" required style="width:100%;margin-bottom:.6rem;padding:.6rem .75rem;background:var(--bg-secondary,#0b120e);color:var(--text-primary,#f5f6f8);border:1px solid var(--border,#22302a);border-radius:8px;font:inherit">
    <input type="text" id="checkin-note" placeholder="Anything else? Deal, quality, vibe (optional)" maxlength="200" style="width:100%;margin-bottom:.6rem;padding:.6rem .75rem;background:var(--bg-secondary,#0b120e);color:var(--text-primary,#f5f6f8);border:1px solid var(--border,#22302a);border-radius:8px;font:inherit">
    <input type="text" id="checkin-name" placeholder="First name or initials (optional)" maxlength="40" style="width:100%;margin-bottom:.7rem;padding:.6rem .75rem;background:var(--bg-secondary,#0b120e);color:var(--text-primary,#f5f6f8);border:1px solid var(--border,#22302a);border-radius:8px;font:inherit">
    <button type="submit" class="cta" style="border:none;cursor:pointer;font:inherit;width:100%">Send it</button>
    <p id="checkin-err" hidden style="color:#f87171;font-size:.8rem;margin:.5rem 0 0;text-align:center"></p>
  </form>
  <div id="checkin-thanks" hidden style="max-width:440px;text-align:center;padding:.8rem;background:var(--bg-card,rgba(255,255,255,0.03));border:1px solid var(--border,#22302a);border-radius:12px;margin-bottom:1.2rem">
    <p style="margin:0;font-size:.9rem">🙏 Got it — it'll appear here once a human checks it. You just made the price map better.</p>
  </div>
</div>
<script>
(function () {
  var slug = ${JSON.stringify(d.id)};
  var W = 'https://dashboard.twincitycannabis.com';
  // Count this page view for the owner's dashboard. These static pages are
  // where search traffic actually lands, and until 2026-09-13 they fired no
  // tracker at all — so owners saw "5 profile views" while Google sent
  // readers here all day. Fire-and-forget; never blocks rendering.
  try {
    fetch(W + '/track', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: slug, event: 'view' }),
      keepalive: true,
    }).catch(function () {});
  } catch (e) {}
  // Outbound clicks to the shop's own site are the conversion that matters.
  document.querySelectorAll('a[rel~="nofollow"][target="_blank"]').forEach(function (a) {
    a.addEventListener('click', function () {
      try {
        fetch(W + '/track', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: slug, event: 'outbound' }),
          keepalive: true,
        }).catch(function () {});
      } catch (e) {}
    });
  });
  fetch(W + '/checkins/public?shop=' + slug).then(function (r) { return r.json(); }).then(function (rows) {
    if (!rows || !rows.length) return;
    document.getElementById('checkins-title').hidden = false;
    document.getElementById('checkins-list').innerHTML = rows.map(function (c) {
      var when = c.submitted_at ? new Date(c.submitted_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '';
      var who = c.name ? String(c.name).replace(/[<>&"]/g, '') : 'A shopper';
      var prod = String(c.product).replace(/[<>&"]/g, '');
      var note = c.note ? ' — “' + String(c.note).replace(/[<>&"]/g, '') + '”' : '';
      return '<div style="padding:.55rem 0;border-bottom:1px solid var(--border,#22302a);font-size:.9rem"><strong style="color:var(--green-text,#4ade80)">$' + Number(c.price).toFixed(2) + '</strong> — ' + prod + note + ' <span style="color:var(--text-muted,#8b909a);font-size:.78rem">· ' + who + ', ' + when + '</span></div>';
    }).join('');
  }).catch(function () {});
  ...
  form.addEventListener('submit', function (e) {
    ...
    err.hidden = true;
    var product = document.getElementById('checkin-product').value.trim();
    var price = parseFloat(document.getElementById('checkin-price').value);
    if (product.length < 2 || !(price > 0.5 && price < 500)) {
      err.textContent = 'A product and a real price are all we need.';
      err.hidden = false;
      return;
    }
    ...
    fetch(W + '/checkin', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        shop: slug, product: product, price: price,
      ...
      form.hidden = true;
      document.getElementById('checkin-thanks').hidden = false;
    }).catch(function () {
...
const buildEventsPage = () => {
  ...
  const schema = [{
    ...
    // Legacy Cup gets full Event schema — the one event with real search
    // volume ("legacy cup 2026") and rich-result potential.
    '@context': 'https://schema.org',
    '@type': 'Festival',
    name: 'Legacy Cup Minnesota 2026',
    description: 'The Midwest’s largest cannabis festival: The Cup Awards, live music from Big Boi, Bone Thugs-N-Harmony, Nappy Roots and Sa-Roc, contests, skate demos, and 12,000+ attendees. 21+.',
    startDate: '2026-09-26T12:00:00-05:00',
    endDate: '2026-09-26T20:00:00-05:00',
    eventStatus: 'https://schema.org/EventScheduled',
    eventAttendanceMode: 'https://schema.org/OfflineEventAttendanceMode',
    location: { '@type': 'Place', name: 'Surly Festival Field', address: { '@type': 'PostalAddress', addressLocality: 'Minneapolis', addressRegion: 'MN', addressCountry: 'US' } },
    performer: [
      { '@type': 'MusicGroup', name: 'Big Boi' },
      { '@type': 'MusicGroup', name: 'Bone Thugs-N-Harmony' },
      { '@type': 'MusicGroup', name: 'Nappy Roots' },
      { '@type': 'MusicGroup', name: 'Sa-Roc' },
    ],
    organizer: { '@type': 'Organization', name: 'Legacy Cup', url: 'https://legacycupmn.com' },
    offers: { '@type': 'Offer', url: 'https://legacycupmn.com/pages/passes', availability: 'https://schema.org/InStock' },
  }];
...
<article class="event-card" data-event-date="2026-05-03">
  <div class="event-date">
...
<!-- Legacy Cup — FEATURED: the big consumer event of the year -->
<article class="event-card event-featured" data-event-date="2026-09-26">
  ...
  <div class="event-body">
    <div class="event-type"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3l1.9 5.1L19 10l-5.1 1.9L12 17l-1.9-5.1L5 10l5.1-1.9z"/><path d="M19 3v4M17 5h4M5 17v2M4 18h2"/></svg> Festival &middot; Competition &middot; 21+</div>
    <div class="event-title">Legacy Cup Minnesota 2026</div>
    <p class="event-tagline">The Midwest&rsquo;s largest cannabis festival &mdash; year six.</p>
    <div class="event-meta">
      <span><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 7.5V12l3 1.8"/></svg> Sat 12:00 &ndash; 8:00 PM</span>
    ...
    <p class="event-desc"><strong>Big Boi (OutKast), Bone Thugs-N-Harmony, Nappy Roots, and Sa-Roc</strong> headline. The Cup Awards crown the best cannabis in Minnesota &mdash; home growers judged alongside licensed and hemp brands &mdash; plus joint-rolling contests, bong pong, pro skate demos, live glassblowing, and 12,000+ of your soon-to-be closest friends. The sampling tent requires a separate ticket or VIP pass.</p>
    <p class="event-audience">If you go to one cannabis event in Minnesota this year, this is the one.</p>
    <a class="event-cta" href="https://legacycupmn.com/pages/passes" rel="noopener" target="_blank">Get passes &rarr;</a>
    <a class="event-cta-ig" href="https://legacycupmn.com/pages/festival" rel="noopener" target="_blank">&#9656; Full festival details</a>
  </div>
</article>
...
<article class="event-card" data-event-date="2026-10-10">
  ...
  <div class="event-body">
    <div class="event-type"><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M17 11h1a3 3 0 0 1 0 6h-1"/><path d="M9 12v6M13 12v6"/><path d="M5 8h12v9a4 4 0 0 1-4 4H9a4 4 0 0 1-4-4z"/><path d="M6 2v2M10 2v2M14 2v2"/></svg> Beer &amp; THC Beverages</div>
    <div class="event-title">Autumn Brew Review</div>
    <div class="event-meta">
      <span><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 7.5V12l3 1.8"/></svg> Sat 1:30 &ndash; 5:00 PM</span>
      <span><svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 21s-6.5-5.8-6.5-10.5a6.5 6.5 0 0 1 13 0C18.5 15.2 12 21 12 21z"/><circle cx="12" cy="10.5" r="2.3"/></svg> Boom Island Park, Minneapolis</span>
    </div>
...
<div class="org-grid">
  <div class="org-card">
    <h3>Canna Club Happy Hour</h3>
    <p>Every Wednesday, 5&ndash;9 PM at Earl Giles Distillery (mezzanine), Minneapolis. THC beverages from Flying High Cannabis Co. plus rotating product samplings from collaborating brands.</p>
    <a href="https://www.minneapolis.org/things-to-do/cannabis/events/" rel="nofollow noopener" target="_blank">Details &rarr;</a>
  </div>
  <div class="org-card">
